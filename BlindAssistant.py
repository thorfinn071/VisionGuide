...
# OpenCV sometimes causes micro-stutters with multithreading; 1 thread is often smoother
try:
    cv2.setNumThreads(1)
except Exception:
    pass

# cuDNN autotune (good if imgsz is fixed and you're on CUDA)
...
# ✅ If True, "left/right" are spoken from the user's POV (after unmirror, swap back)
...
ROI_WIDTH_RATIO = 0.6          # center 60% width
ROI_HEIGHT_RATIO = 0.65        # bottom 65% height
FULL_FRAME_REFRESH_SEC = 1.5   # full frame every N seconds
...
# Speech cooldowns (still used for dedup / logic)
SPEECH_COOLDOWN = 2.5
PERSON_COOLDOWN = 7.0
FREE_PATH_COOLDOWN = 4.0

# 2.4 confirmation (anti one-frame glitches)
CLOSE_CONFIRM_FRAMES = 2
VERY_CLOSE_CONFIRM_FRAMES = 2

# Speech priorities (2.0)
PRIORITY_CRITICAL = 0   # very close warnings (can purge queue)
PRIORITY_IMPORTANT = 1  # guidance / scan answers / mode changes
PRIORITY_INFO = 2       # summaries, low-priority info

# ✅ anti-spam for critical repeated warnings (same text)
CRITICAL_MIN_GAP = 1.4
_last_critical_text = ""
_last_critical_time = 0.0

# ===== Approaching vehicle detection (cheap, bbox growth) =====
APPROACH_HIST_LEN = 6
APPROACH_MIN_DT = 0.35
VEH_APPROACH_AREA_RATE_T = 0.22     # area_ratio per second
VEH_APPROACH_HEIGHT_RATE_T = 0.14   # height_ratio per second
APPROACH_THREAT_BOOST_MAX = 0.75    # max +75% threat boost
APPROACH_SAY_COOLDOWN = 1.4
last_approach_say_time = 0.0

# ===== Walkway / corridor hint (cheap edges) =====
WALKWAY_ENABLE = True
...
# ===== State machine (CLEAR / CAUTION / DANGER) =====
STATE_CLEAR = "clear"
STATE_CAUTION = "caution"
STATE_DANGER = "danger"

# How long condition must hold before we switch states
CLEAR_HOLD_SEC = 1.2
CAUTION_HOLD_SEC = 0.25
DANGER_HOLD_SEC = 0.12

# Don't say "clear" if YOLO hasn't published recently (thread dead / stale)
YOLO_STALE_SEC = 2.0
...
def get_horizontal_position(cx, w):
    ...
    # If we flipped the image, and we want user POV, swap sides back
...
def walkway_hint_from_frame(frame_bgr):
    """
    Returns: (hint_str, x_norm, confidence)
    hint_str: "left" / "right" / "forward"
    """
    try:
...
# ================= SPEECH (single-thread COM + interrupt via Event) =================
...
def _drop_pending_info():
    """Remove pending INFO messages from the queue, keep CRITICAL/IMPORTANT."""
    try:
        temp = []
        while not speech_queue.empty():
...
def say(text, priority=PRIORITY_INFO, purge=False):
    ...
    # Critical anti-spam (same critical text too often)
    ...
    # If speech backlog is large, skip low-priority INFO to avoid voice lag
    if priority >= PRIORITY_INFO:
        try:
    ...
    # Debounce duplicates (but allow CRITICAL through even if repeated fast)
    if priority != PRIORITY_CRITICAL:
    ...
    try:
        ...
        # If full: try to drop one INFO first; if still full and this is INFO => skip
...
# ================= CAMERA (MSMF first + MJPG + watchdog) =================
...
CAM_STALL_SEC = 1.5  # watchdog threshold

# ✅ cap_lock prevents race when restarting camera
...
last_yolo_publish_time = 0.0  # updated by yolo_worker
...
def match_tracks(dets, existing_tracks):
    ...
    for di in range(len(dets)):
        ...
        for tj in range(len(track_ids)):
            tid = track_ids[tj]
            # важное: матчим только одинаковый класс
...
def yolo_worker():
    ...
    while yolo_running:
        ...
        try:
            ...
            if now - getattr(yolo_worker, "_last_err_t", 0) > 2.0:
                print("YOLO worker error:", repr(e))
                yolo_worker._last_err_t = now
            continue
        ...
        for r in results:
            ...
            for i, conf in enumerate(confidences):
                ...
                # ✅ record side activity BEFORE filtering (street often filters sides)
                if pos != "center":
        ...
        for di in new_det_indices:
            ...
            tracks[tid] = {
                ...
                # approach histories created lazily
            }

        # build active list + approaching logic
        active = []
        for tid, t in tracks.items():
            if t["age"] != 0:
                continue
...
# --- Stable empty-scene gating (anti false "Path is clear") ---
EMPTY_SCENE_CONFIRM_SEC = 1.2        # must be empty this long
CLEAR_MIN_GAP_AFTER_OBJECT = 1.4     # after last seen object before "clear"
...
try:
    ...
    while True:
        ...
        # --- Camera watchdog (restart capture if camera stalls) ---
        ...
        # Draw only on a resized copy (cheap) to avoid touching lf
        display_counter += 1
        ...
        # needed for guidance even when YOLO is not updated this frame
        h0, w0 = lf.shape[:2]
        frame_area = h0 * w0
        ...
        with detections_lock:
            ...
            # --- reset clear flag if any object appears ---
            if detections:
                clear_announced = False

        # --- update empty/object timers ---
        if detections:
        ...
        # ---- Walkway hint update (cheap edges) ----
        ...
        if now - last_yolo_publish_time > YOLO_STALE_SEC:
            desired_state = STATE_CAUTION  # conservative
        else:
        ...
        # --- Danger state: say ONLY "Warning." once on entry ---
        danger_just_announced = False
        ...
        if current_mode == MODE_CANE:
            ...
            # --- approaching vehicle speech (important, not spam) ---
        ...
        # give time to other threads (YOLO, speech)
        now_ui = time.time()
